...
int minTimeToReach(int** moveTime, int moveTimeSize, int* moveTimeColSize)
{
    ...
    unsigned char visited[750][100]={0};
    struct node temp;
    const int INF=2e9;
    m=moveTimeSize;
    n=moveTimeColSize[0];
    ...
    insert(minheap,(struct node){0,0,0,true});
    while(minheap->size)
    {
        temp=top(minheap);
        pop(minheap);
        ...
        for(i=0;i<4;i++)
        {
            ...
            if(nx>=0&&nx<m&&ny>=0&&ny<n)
            {
                if(temp.odd)
                {
                    ...
                    {
                        d[nx][ny]=dist;
                        insert(minheap,(struct node){nx,ny,dist,false});
                    }
                }
                else
                {
                    dist=maxi(t,moveTime[nx][ny])+2;
                    if(dist<d[nx][ny])
                    {
                        d[nx][ny]=dist;
                        insert(minheap,(struct node){nx,ny,dist,true});
                    }
                }
            }
        }
